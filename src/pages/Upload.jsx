...
import AIPolicyReviewExperience from '../components/AIPolicyReviewExperience.jsx'
...
export default function Upload() {
  ...
  const { analysis, setAnalysis, addToHistory } = usePolicy()
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      try {
        ...
        result = await analyzeWithLLM(text, { fileName: file.name, truncated })
      } catch (err) {
        ...
        result = analyzeText(text, { fileName: file.name, truncated })
      }
  ...
  async function downloadPdf() {
    ...
    try {
      // jsPDF is a large dependency and most uploads never trigger a
      // download, so load it only when actually requested rather than
      // paying its bundle cost on every visit to /upload.
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      ...
      {(state === 'reading' || state === 'scanning' || state === 'done') && (
        <div className="mt-8">
